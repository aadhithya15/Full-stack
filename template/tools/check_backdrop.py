...
from bgmask import background, body_silhouette
...
files=sorted(set([os.path.join(d,os.path.basename(p)) for p in sorted(glob.glob('template/base/*.jpg'))
                  for d in [os.environ.get('TPL_BASE','template/base')]+ [f"{os.environ.get('TPL_ROOT','template')}/{t}" for t in TONE]]))
...
for p in files:
    ...
    solidB=ndimage.binary_fill_holes(body_silhouette(X))
    bg,_=background(X); bg = bg & ~ndimage.binary_dilation(solidB,np.ones((3,3)),iterations=2)
